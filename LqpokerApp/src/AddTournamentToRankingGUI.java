			...
			public void actionPerformed(ActionEvent arg0) {
...

				{
					...
					listCurrentTournament.replaceItem(
						...
						Player tempPlayer = db.getSinglePlayerFromJsonFile(name,middleNames, surname);
						//db.appendNameToJsonFile(comboBoxAddPlayerTpTournament.getSelectedItem().toString(), filePath);
						if(  tempPlayer  == null ) {
							tempPlayer.setname(name);
							tempPlayer.setmiddleName(middleNames);
							tempPlayer.setsurname(surname);
							tempPlayer.setEditor(false);
							tempPlayer.setpassword("");
							tempPlayer.setusername("");
							db.setSinglePlayerToJsonFile(tempPlayer);
						}
